...
export function setSession(res:VercelResponse,s:Session){const iv=crypto.randomBytes(12), c=crypto.createCipheriv('aes-256-gcm',key() as any,iv); const body=Buffer.concat([c.update(JSON.stringify(s)),c.final()]); const value=[iv, c.getAuthTag(),body].map(x=>x.toString('base64url')).join('.'); res.setHeader('Set-Cookie',`watch_session=${value}; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=2592000`)}
