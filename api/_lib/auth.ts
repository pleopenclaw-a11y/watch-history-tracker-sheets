...
export function setSession(res:VercelResponse,s:Session){const iv=crypto.randomBytes(12), c=crypto.createCipheriv('aes-256-gcm',key() as any,iv as any); const body=Buffer.concat([c.update(JSON.stringify(s)) as any,c.final() as any]); const value=([iv, c.getAuthTag(),body] as any[]).map(x=>x.toString('base64url')).join('.'); res.setHeader('Set-Cookie',`watch_session=${value}; HttpOnly; Secure; SameSite=Lax; Path=/; Max-Age=2592000`)}
...
export function session(req:VercelRequest){try{const raw=String(req.headers.cookie||'').split(';').map(x=>x.trim()).find(x=>x.startsWith('watch_session='))?.slice(14); if(!raw)return null; const [i,t,b]=raw.split('.'); const d=crypto.createDecipheriv('aes-256-gcm',key() as any,Buffer.from(i,'base64url') as any); d.setAuthTag(Buffer.from(t,'base64url') as any); return JSON.parse(Buffer.concat([d.update(Buffer.from(b,'base64url') as any) as any,d.final() as any]).toString()) as Session}catch{return null}}
