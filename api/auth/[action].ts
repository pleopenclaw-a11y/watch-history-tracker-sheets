...
import { oauth,json,method,setSession,clearSession,session } from '../_lib/auth';
export default async function handler(req:VercelRequest,res:VercelResponse){
 ...
 if(req.url?.includes('/callback')){if(!method(req,res,'GET'))return;try{const code=String(req.query.code||'');if(!code)return json(res,400,{error:'missing_code'});const client=oauth(),{tokens}=await client.getToken(code);client.setCredentials(tokens);const {data}=await google.oauth2({version:'v2',auth:client}).userinfo.get();if(!tokens.refresh_token)return json(res,400,{error:'missing_refresh_token'});setSession(res,{sub:data.id!,email:data.email!,name:data.name||undefined,refreshToken:tokens.refresh_token});return res.redirect('/')}catch(e){return json(res,500,{error:'oauth_callback_failed'})}}
